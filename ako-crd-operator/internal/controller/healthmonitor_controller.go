...
func (r *HealthMonitorReconciler) ReconcileIfRequired(ctx context.Context, hm *akov1alpha1.HealthMonitor) error {
	...
	if hm.Status.Uuid == "" {
		if err := r.AviClient.AviSession.Post(utils.GetUriEncoded("/api/healthmonitor"), hm.Spec, &resp); err != nil {
			...
			if aviError, ok := err.(session.AviError); ok {
				if aviError.HttpStatusCode == http.StatusConflict {
					...
					if err != nil {
						utils.AviLog.Error(err, "error getting healthmonitor")
						return err
					}
					utils.AviLog.Infof("healthmonitor uuid: %v", resp)
				} else {
					return err
				}
			} else {
		...
		if !ok {
			fmt.Println("Error: 'results' not found or not an array")
			return fmt.Errorf("'results' not found or not an array")
		}

		// Check if the results array is empty
		if len(results) == 0 {
			fmt.Println("Error: 'results' array is empty")
		...
		if !ok {
			fmt.Println("Error: first element in 'results' is not a map")
		...
		if !ok {
			fmt.Println("Error: 'uuid' not found or not a string")
