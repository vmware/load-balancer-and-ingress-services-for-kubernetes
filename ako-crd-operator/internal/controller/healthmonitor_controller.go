...
import (
	...
	lib "github.com/vmware/load-balancer-and-ingress-services-for-kubernetes/ako-crd-operator/internal/lib"
...
func (r *HealthMonitorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if hm.ObjectMeta.DeletionTimestamp.IsZero() {
		if !controllerutil.ContainsFinalizer(hm, lib.HealthMonitorFinalizer) {
			patch := client.MergeFrom(hm.DeepCopy())
			controllerutil.AddFinalizer(hm, lib.HealthMonitorFinalizer)
		...
		if removeFinalizer {
			patch := client.MergeFrom(hm.DeepCopy())
			controllerutil.RemoveFinalizer(hm, lib.HealthMonitorFinalizer)
			if err := r.Patch(ctx, hm, patch); err != nil {
				return ctrl.Result{}, err
			}
		} else {
			...
			return ctrl.Result{RequeueAfter: lib.RequeueInterval}, nil
		}
		return ctrl.Result{}, nil
	}
...
func healthMonitorSecretPredicate(obj client.Object) bool {
	...
	return secret.Type == lib.HealthMonitorSecretType
}
...
func (r *HealthMonitorReconciler) DeleteObject(ctx context.Context, hm *akov1alpha1.HealthMonitor) (error, bool) {
	...
	if hm.Status.UUID != "" && hm.Status.Tenant != "" {
		if err := r.AviClient.AviSessionDelete(fmt.Sprintf("%s/%s", lib.HealthMonitorURL, hm.Status.UUID), nil, nil, session.SetOptTenant(hm.Status.Tenant)); err != nil {
...
func (r *HealthMonitorReconciler) ReconcileIfRequired(ctx context.Context, hm *akov1alpha1.HealthMonitor) error {
	...
	hmReq := &HealthMonitorRequest{
		Name:              lib.GetObjectName(hm.Namespace, hm.Name),
	...
	if hm.Status.UUID == "" {
		...
		if err := r.AviClient.AviSessionPut(utils.GetUriEncoded(fmt.Sprintf("%s/%s", lib.HealthMonitorURL, hm.Status.UUID)), hmReq, &resp, session.SetOptTenant(namespaceTenant)); err != nil {
...
func (r *HealthMonitorReconciler) createHealthMonitor(ctx context.Context, hmReq *HealthMonitorRequest, hm *akov1alpha1.HealthMonitor, tenant string) (map[string]interface{}, error) {
	log := utils.LoggerFromContext(ctx)
	resp := map[string]interface{}{}
	if err := r.AviClient.AviSessionPost(utils.GetUriEncoded(lib.HealthMonitorURL), hmReq, &resp, session.SetOptTenant(tenant)); err != nil {
		...
		if aviError, ok := err.(session.AviError); ok {
			if aviError.HttpStatusCode == http.StatusConflict && strings.Contains(aviError.Error(), "already exists") {
				...
				err := r.AviClient.AviSessionGet(utils.GetUriEncoded(fmt.Sprintf("%s?name=%s", lib.HealthMonitorURL, hmReq.Name)), &resp, session.SetOptTenant(tenant))
				if err != nil {
				...
				if err := r.AviClient.AviSessionPut(utils.GetUriEncoded(fmt.Sprintf("%s/%s", lib.HealthMonitorURL, uuid)), hmReq, &resp, session.SetOptTenant(tenant)); err != nil {
					log.Errorf("error updating healthmonitor: %s", err.Error())
					return nil, err
				}
				hm.Status.Conditions = controllerutils.SetCondition(hm.Status.Conditions, metav1.Condition{
...
func (r *HealthMonitorReconciler) resolveSecret(ctx context.Context, namespace string, hmReq *HealthMonitorRequest) (string, error) {
	...
	if secret.Type != lib.HealthMonitorSecretType {
		log.Errorf("secret is not of %s type", lib.HealthMonitorSecretType)
		return "", akoerrors.AKOCRDOperatorError{
			HttpStatusCode: http.StatusBadRequest,
			Reason:         "ConfigurationError",
			Message:        fmt.Sprintf("Secret %s is not of type %s", hmReq.Authentication.SecretRef, lib.HealthMonitorSecretType),
		}
	}
