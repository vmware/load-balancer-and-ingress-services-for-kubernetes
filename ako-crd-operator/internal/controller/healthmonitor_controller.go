		...
		if err := r.AviClient.AviSessionDelete(fmt.Sprintf("%s/%s", constants.HealthMonitorURL, hm.Status.UUID), nil, nil, session.SetOptTenant(hm.Status.Tenant)); err != nil {
			...
			if aviError, ok := err.(session.AviError); ok {
				...
				case 403:
					...
					if err := r.StatusManager.SetStatus(ctx, hm, akov1alpha1.ObjectConditionDeleted, metav1.ConditionFalse, akov1alpha1.ObjectReasonDeletionSkipped, controllerutils.ParseAviErrorMessage(*aviError.Message)); err != nil {
						return err, false
					}
					return nil, false
				}
...
func (r *HealthMonitorReconciler) ReconcileIfRequired(ctx context.Context, hm *akov1alpha1.HealthMonitor) error {
	...
	if err != nil {
		if statusErr := r.StatusManager.SetStatus(ctx, hm, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonCreationFailed, fmt.Sprintf("Error getting tenant in namespace: %s", err.Error())); statusErr != nil {
			return statusErr
		}
	...
	if hm.Status.UUID == "" {
		...
		if err != nil {
			if statusErr := r.StatusManager.SetStatus(ctx, hm, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonCreationFailed, fmt.Sprintf("Failed to create HealthMonitor on Avi Controller: %v", err)); statusErr != nil {
				return statusErr
			}
		...
		if err != nil {
			if statusErr := r.StatusManager.SetStatus(ctx, hm, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonUUIDExtractionFailed, fmt.Sprintf("Failed to extract UUID: %v", err)); statusErr != nil {
				return statusErr
			}
		...
		if err := r.StatusManager.SetStatus(ctx, hm, akov1alpha1.ObjectConditionReady, metav1.ConditionTrue, akov1alpha1.ObjectReasonCreated, "HealthMonitor created successfully on Avi Controller"); err != nil {
			return err
		}
		...
		if err := r.AviClient.AviSessionPut(utils.GetUriEncoded(fmt.Sprintf("%s/%s", constants.HealthMonitorURL, hm.Status.UUID)), hmReq, &resp, session.SetOptTenant(namespaceTenant)); err != nil {
			...
			if statusErr := r.StatusManager.SetStatus(ctx, hm, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonUpdateFailed, fmt.Sprintf("Failed to update HealthMonitor on Avi Controller: %v", err)); statusErr != nil {
				return statusErr
			}
			return err
		}
		...
		hm.Status.BackendObjectName = hmReq.Name
		hm.Status.Tenant = namespaceTenant
		if err := r.StatusManager.SetStatus(ctx, hm, akov1alpha1.ObjectConditionReady, metav1.ConditionTrue, akov1alpha1.ObjectReasonUpdated, "HealthMonitor updated successfully on Avi Controller"); err != nil {
			return err
		}
