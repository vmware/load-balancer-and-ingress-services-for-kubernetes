...
func (r *PKIProfileReconciler) DeleteObject(ctx context.Context, pki *akov1alpha1.PKIProfile) (error, bool) {
	...
	if pki.Status.UUID != "" && pki.Status.Tenant != "" {
		if err := r.AviClient.AviSessionDelete(utils.GetUriEncoded(fmt.Sprintf("%s/%s", constants.PKIProfileURL, pki.Status.UUID)), nil, nil, session.SetOptTenant(pki.Status.Tenant)); err != nil {
			...
			if aviError, ok := err.(session.AviError); ok {
				...
				case 403:
					...
					if err := r.SetStatus(ctx, pki, akov1alpha1.ObjectConditionDeleted, metav1.ConditionFalse, akov1alpha1.ObjectReasonDeletionSkipped, controllerutils.ParseAviErrorMessage(*aviError.Message)); err != nil {
						return err, false
					}
					return nil, false
				}
			...
			if statusErr := r.SetStatus(ctx, pki, akov1alpha1.ObjectConditionDeleted, metav1.ConditionFalse, akov1alpha1.ObjectReasonDeletionFailed, fmt.Sprintf("Failed to delete PKIProfile from Avi Controller: %v", err)); statusErr != nil {
		...
		if err := r.SetStatus(ctx, pki, akov1alpha1.ObjectConditionDeleted, metav1.ConditionTrue, akov1alpha1.ObjectReasonDeletionSkipped, "UUID not present, PKIProfile may not have been created on Avi Controller"); err != nil {
			return err, false
		}
	}
	return nil, true
...
func (r *PKIProfileReconciler) ReconcileIfRequired(ctx context.Context, pki *akov1alpha1.PKIProfile) error {
	...
	if err != nil {
		if statusErr := r.StatusManager.SetStatus(ctx, pki, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonCreationFailed, fmt.Sprintf("Error getting tenant in namespace: %s", err.Error())); statusErr != nil {
			return statusErr
		}
	...
	if pki.Status.Tenant != "" && pki.Status.Tenant != namespaceTenant {
		...
		if err != nil {
			...
			if statusErr := r.SetStatus(ctx, pki, akov1alpha1.ObjectConditionDeleted, metav1.ConditionFalse, akov1alpha1.ObjectReasonDeletionFailed, fmt.Sprintf("Failed to delete PKIProfile due to error: %v", err)); statusErr != nil {
				return statusErr
			}
			return err
		}
	...
	if pki.Status.UUID == "" {
		...
		if err != nil {
			...
			if statusErr := r.SetStatus(ctx, pki, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonCreationFailed, fmt.Sprintf("Failed to create PKIProfile on Avi Controller: %v", err)); statusErr != nil {
				return statusErr
			}
			return err
		}
		...
		if err != nil {
			...
			if statusErr := r.SetStatus(ctx, pki, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonUUIDExtractionFailed, fmt.Sprintf("Failed to extract UUID: %v", err)); statusErr != nil {
				return statusErr
			}
			return err
		}
		...
		if err := r.SetStatus(ctx, pki, akov1alpha1.ObjectConditionReady, metav1.ConditionTrue, akov1alpha1.ObjectReasonCreated, "PKIProfile created successfully on Avi Controller"); err != nil {
			return err
		}
	} else {
		// this is a PUT Call
		...
		if err := r.AviClient.AviSessionPut(utils.GetUriEncoded(fmt.Sprintf("%s/%s", constants.PKIProfileURL, pki.Status.UUID)), pkiReq, &resp, session.SetOptTenant(namespaceTenant)); err != nil {
			...
			if statusErr := r.SetStatus(ctx, pki, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonUpdateFailed, fmt.Sprintf("Failed to update PKIProfile on Avi Controller: %v", err)); statusErr != nil {
				return statusErr
			}
			return err
		}
		pki.Status.BackendObjectName = *pkiReq.Name
		pki.Status.Tenant = namespaceTenant
		if err := r.SetStatus(ctx, pki, akov1alpha1.ObjectConditionReady, metav1.ConditionTrue, akov1alpha1.ObjectReasonUpdated, "PKIProfile updated successfully on Avi Controller"); err != nil {
			return err
		}
	}
	return nil
