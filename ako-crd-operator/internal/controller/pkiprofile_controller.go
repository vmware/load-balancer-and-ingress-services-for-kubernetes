...
func (r *PKIProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if pki.ObjectMeta.DeletionTimestamp.IsZero() {
		if !controllerutil.ContainsFinalizer(pki, lib.PKIProfileFinalizer) {
			controllerutil.AddFinalizer(pki, lib.PKIProfileFinalizer)
		...
		if removeFinalizer {
			controllerutil.RemoveFinalizer(pki, lib.PKIProfileFinalizer)
			if err := r.Patch(ctx, pki, patch); err != nil {
				return ctrl.Result{}, err
			}
		} else {
...
func (r *PKIProfileReconciler) DeleteObject(ctx context.Context, pki *akov1alpha1.PKIProfile) (error, bool) {
	...
	if pki.Status.UUID != "" && pki.Status.Tenant != "" {
		if err := r.AviClient.AviSessionDelete(utils.GetUriEncoded(fmt.Sprintf("%s/%s", lib.PKIProfileURL, pki.Status.UUID)), nil, nil, session.SetOptTenant(pki.Status.Tenant)); err != nil {
...
func (r *PKIProfileReconciler) ReconcileIfRequired(ctx context.Context, pki *akov1alpha1.PKIProfile) error {
	...
	name := lib.GetObjectName(pki.Namespace, pki.Name)
	...
	if pki.Status.UUID == "" {
		...
		if err := r.AviClient.AviSessionPut(utils.GetUriEncoded(fmt.Sprintf("%s/%s", lib.PKIProfileURL, pki.Status.UUID)), pkiReq, &resp, session.SetOptTenant(namespaceTenant)); err != nil {
...
func (r *PKIProfileReconciler) createPKIProfile(ctx context.Context, pkiReq *models.PKIprofile, pki *akov1alpha1.PKIProfile, tenant string) (map[string]interface{}, error) {
	log := utils.LoggerFromContext(ctx)
	resp := map[string]interface{}{}
	if err := r.AviClient.AviSessionPost(utils.GetUriEncoded(lib.PKIProfileURL), pkiReq, &resp, session.SetOptTenant(tenant)); err != nil {
		...
		if aviError, ok := err.(session.AviError); ok {
			if aviError.HttpStatusCode == http.StatusConflict && strings.Contains(aviError.Error(), "already exists") {
				...
				err := r.AviClient.AviSessionGet(utils.GetUriEncoded(fmt.Sprintf("%s?name=%s", lib.PKIProfileURL, *pkiReq.Name)), &resp, session.SetOptTenant(tenant))
				if err != nil {
				...
				if err := r.AviClient.AviSessionPut(utils.GetUriEncoded(fmt.Sprintf("%s/%s", lib.PKIProfileURL, uuid)), pkiReq, &resp, session.SetOptTenant(tenant)); err != nil {
