...
func TestRouteBackendExtensionBackendTLSValidation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			rbe: &akov1alpha1.RouteBackendExtension{
				...
				Spec: akov1alpha1.RouteBackendExtensionSpec{
					...
					DomainName:       func() *string { s := "example.com"; return &s }(),
				},
			},
			wantErr:      false,
			wantStatus:   constants.ACCEPTED,
		...
		},
		{
			...
			wantErr:      true,
			wantStatus:   constants.REJECTED,
			wantErrorMsg: "pkiProfile can only be configured when enableBackendSSL is set to true",
		},
		{
		...
		},
		{
			...
			wantErr:      true,
			wantStatus:   constants.REJECTED,
			wantErrorMsg: "PKIProfile not-ready-pki-profile is not ready in namespace default",
		},
		{
			...
			wantErr:      true,
			wantStatus:   constants.REJECTED,
			wantErrorMsg: "PKIProfile no-conditions-pki-profile is not ready in namespace default",
		},
	}
