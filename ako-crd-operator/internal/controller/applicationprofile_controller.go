		...
		if err := r.AviClient.AviSessionDelete(utils.GetUriEncoded(fmt.Sprintf("%s/%s", constants.ApplicationProfileURL, ap.Status.UUID)), nil, nil, session.SetOptTenant(ap.Status.Tenant)); err != nil {
			...
			if aviError, ok := err.(session.AviError); ok {
				...
				case 403:
					...
					if err := r.StatusManager.SetStatus(ctx, ap, akov1alpha1.ObjectConditionDeleted, metav1.ConditionFalse, akov1alpha1.ObjectReasonDeletionSkipped, controllerutils.ParseAviErrorMessage(*aviError.Message)); err != nil {
						return err, false
					}
					return nil, false
				}
...
func (r *ApplicationProfileReconciler) ReconcileIfRequired(ctx context.Context, ap *akov1alpha1.ApplicationProfile) error {
	...
	if err != nil {
		if statusErr := r.StatusManager.SetStatus(ctx, ap, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonCreationFailed, fmt.Sprintf("Error getting tenant in namespace: %s", err.Error())); statusErr != nil {
			return statusErr
		}
	...
	if ap.Status.UUID == "" {
		...
		if err != nil {
			if statusErr := r.StatusManager.SetStatus(ctx, ap, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonCreationFailed, fmt.Sprintf("Failed to create ApplicationProfile on Avi Controller: %v", err)); statusErr != nil {
				return statusErr
			}
		...
		if err != nil {
			if statusErr := r.StatusManager.SetStatus(ctx, ap, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonUUIDExtractionFailed, fmt.Sprintf("Failed to extract UUID: %v", err)); statusErr != nil {
				return statusErr
			}
		...
		if err := r.StatusManager.SetStatus(ctx, ap, akov1alpha1.ObjectConditionReady, metav1.ConditionTrue, akov1alpha1.ObjectReasonCreated, "ApplicationProfile created successfully on Avi Controller"); err != nil {
			return err
		}
		...
		if err := r.AviClient.AviSessionPut(utils.GetUriEncoded(fmt.Sprintf("%s/%s", constants.ApplicationProfileURL, ap.Status.UUID)), apReq, &resp, session.SetOptTenant(namespaceTenant)); err != nil {
			...
			if statusErr := r.StatusManager.SetStatus(ctx, ap, akov1alpha1.ObjectConditionReady, metav1.ConditionFalse, akov1alpha1.ObjectReasonUpdateFailed, fmt.Sprintf("Failed to update ApplicationProfile on Avi Controller: %v", err)); statusErr != nil {
				return statusErr
			}
			return err
		}
		ap.Status.BackendObjectName = apReq.Name
		ap.Status.Tenant = namespaceTenant
		if err := r.StatusManager.SetStatus(ctx, ap, akov1alpha1.ObjectConditionReady, metav1.ConditionTrue, akov1alpha1.ObjectReasonUpdated, "ApplicationProfile updated successfully on Avi Controller"); err != nil {
			return err
		}
