		...
		if err := r.AviClient.AviSessionDelete(utils.GetUriEncoded(fmt.Sprintf("%s/%s", constants.ApplicationProfileURL, ap.Status.UUID)), nil, nil, session.SetOptTenant(ap.Status.Tenant)); err != nil {
			...
			if aviError, ok := err.(session.AviError); ok {
				...
				case 403:
					...
					if err := r.StatusManager.SetStatus(ctx, ap, akov1alpha1.ObjectConditionProgrammed, metav1.ConditionFalse, akov1alpha1.ObjectReasonDeletionSkipped, controllerutils.ParseAviErrorMessage(*aviError.Message)); err != nil {
						return err, false
					}
					return nil, false
				}
...
func (r *ApplicationProfileReconciler) ReconcileIfRequired(ctx context.Context, ap *akov1alpha1.ApplicationProfile) error {
	...
	if err != nil {
		if statusErr := r.StatusManager.SetStatus(ctx, ap, akov1alpha1.ObjectConditionProgrammed, metav1.ConditionFalse, akov1alpha1.ObjectReasonCreationFailed, fmt.Sprintf("Error getting tenant in namespace: %s", err.Error())); statusErr != nil {
			return statusErr
		}
