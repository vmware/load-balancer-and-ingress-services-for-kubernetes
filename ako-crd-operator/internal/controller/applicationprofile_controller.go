...
func (r *ApplicationProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if ap.ObjectMeta.DeletionTimestamp.IsZero() {
		if !controllerutil.ContainsFinalizer(ap, lib.ApplicationProfileFinalizer) {
			patch := client.MergeFrom(ap.DeepCopy())
			controllerutil.AddFinalizer(ap, lib.ApplicationProfileFinalizer)
		...
		if removeFinalizer {
			patch := client.MergeFrom(ap.DeepCopy())
			controllerutil.RemoveFinalizer(ap, lib.ApplicationProfileFinalizer)
			if err := r.Patch(ctx, ap, patch); err != nil {
				return ctrl.Result{}, err
			}
		} else {
...
func (r *ApplicationProfileReconciler) DeleteObject(ctx context.Context, ap *akov1alpha1.ApplicationProfile) (error, bool) {
	...
	if ap.Status.UUID != "" && ap.Status.Tenant != "" {
		if err := r.AviClient.AviSessionDelete(utils.GetUriEncoded(fmt.Sprintf("%s/%s", lib.ApplicationProfileURL, ap.Status.UUID)), nil, nil, session.SetOptTenant(ap.Status.Tenant)); err != nil {
...
func (r *ApplicationProfileReconciler) ReconcileIfRequired(ctx context.Context, ap *akov1alpha1.ApplicationProfile) error {
	...
	apReq := &ApplicationProfileRequest{
		Name:                   lib.GetObjectName(ap.Namespace, ap.Name),
	...
	if ap.Status.UUID == "" {
		...
		if err := r.AviClient.AviSessionPut(utils.GetUriEncoded(fmt.Sprintf("%s/%s", lib.ApplicationProfileURL, ap.Status.UUID)), apReq, &resp, session.SetOptTenant(namespaceTenant)); err != nil {
...
func (r *ApplicationProfileReconciler) createApplicationProfile(ctx context.Context, apReq *ApplicationProfileRequest, ap *akov1alpha1.ApplicationProfile, tenant string) (map[string]interface{}, error) {
	log := utils.LoggerFromContext(ctx)
	resp := map[string]interface{}{}
	if err := r.AviClient.AviSessionPost(utils.GetUriEncoded(lib.ApplicationProfileURL), apReq, &resp, session.SetOptTenant(tenant)); err != nil {
		...
		if aviError, ok := err.(session.AviError); ok {
			if aviError.HttpStatusCode == http.StatusConflict && strings.Contains(aviError.Error(), "already exists") {
				...
				err := r.AviClient.AviSessionGet(utils.GetUriEncoded(fmt.Sprintf("%s?name=%s", lib.ApplicationProfileURL, apReq.Name)), &resp, session.SetOptTenant(tenant))
				if err != nil {
				...
				if err := r.AviClient.AviSessionPut(utils.GetUriEncoded(fmt.Sprintf("%s/%s", lib.ApplicationProfileURL, uuid)), apReq, &resp, session.SetOptTenant(tenant)); err != nil {
