...
func TestPKIProfileController(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			prepare: func(mockAviClient *mock.MockAviClientInterface) {
				mockAviClient.EXPECT().AviSessionPost(constants.PKIProfileURL, gomock.Any(), gomock.Any(), gomock.Any()).Return(errors.New("connection failed")).AnyTimes()
			},
			want: &akov1alpha1.PKIProfile{
				ObjectMeta: metav1.ObjectMeta{
					Name:            "test-error",
					...
					Conditions: []metav1.Condition{
						{
							...
							Message: "Failed to create PKIProfile on Avi Controller: connection failed",
						},
					},
				},
				Spec: akov1alpha1.PKIProfileSpec{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.want.Status.Conditions[0].Message, updatedPKI.Status.Conditions[0].Message)
...
func TestPKIProfileValidatedObject(t *testing.T) {
	tests := []struct {
		name         string
		pki          *akov1alpha1.PKIProfile
		wantErr      bool
		...
		{
			name: "success: multiple valid CACerts",
			pki: &akov1alpha1.PKIProfile{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-pki-multiple",
					Namespace: "default",
				},
				Spec: akov1alpha1.PKIProfileSpec{
					CACerts: []*akov1alpha1.SSLCertificate{
		...
		{
			name: "success: nil CACerts (no validation error)",
			pki: &akov1alpha1.PKIProfile{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-pki-nil",
					Namespace: "default",
		...
		{
			name: "success: empty CACerts (no validation error)",
			pki: &akov1alpha1.PKIProfile{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-pki-empty",
					Namespace: "default",
		...
		{
			name: "error: empty certificate string",
			pki: &akov1alpha1.PKIProfile{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-pki-empty-cert",
					Namespace: "default",
				},
				Spec: akov1alpha1.PKIProfileSpec{
					CACerts: []*akov1alpha1.SSLCertificate{{Certificate: func() *string { s := ""; return &s }()}},
				},
			},
			wantErr:      true,
			wantErrorMsg: "CA certificate at index 0 is empty",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			err := reconciler.ValidatedObject(ctx, tt.pki)
			...
			if tt.wantErr {
				...
				found := false
				for _, condition := range tt.pki.Status.Conditions {
					if condition.Type == "Ready" && condition.Status == "False" && condition.Reason == "ValidationFailed" {
						found = true
						break
					}
				}
				assert.True(t, found, "Expected Ready=False condition with ValidationFailed reason")
			} else {
				// Should have Ready=True condition
				found := false
				for _, condition := range tt.pki.Status.Conditions {
					if condition.Type == "Ready" && condition.Status == "True" && condition.Reason == "ValidationSucceeded" {
						found = true
						break
					}
				}
				assert.True(t, found, "Expected Ready=True condition with ValidationSucceeded reason")
			}
		})
	}
}
