...
func (r *RouteBackendExtensionReconciler) ValidatedObject(ctx context.Context, rbe *akov1alpha1.RouteBackendExtension) error {
	...
	if rbe.Spec.PKIProfile != nil && rbe.Spec.PKIProfile.Kind == akov1alpha1.ObjectKindCRD {
		...
		if !isReady {
			log.Errorf("PKIProfile %s is not ready in namespace %s", rbe.Spec.PKIProfile.Name, rbe.Namespace)
			err = fmt.Errorf("PKIProfile %s is not ready in namespace %s", rbe.Spec.PKIProfile.Name, rbe.Namespace)
			r.SetStatus(rbe, err.Error(), constants.REJECTED)
			return err
		}

		log.Infof("PKIProfile %s is ready in namespace %s", rbe.Spec.PKIProfile.Name, rbe.Namespace)
	}
