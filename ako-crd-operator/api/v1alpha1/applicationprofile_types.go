...
// +kubebuilder:validation:Enum=AUTO_COMPRESSION;CUSTOM_COMPRESSION
type CompressionType string

const (
	//Compression actions based on SE state and Content properties
	AUTO_COMPRESSION CompressionType = "Auto"
	//Compression actions based on custom compression rules
	CUSTOM_COMPRESSION CompressionType = "Custom"
)

// AVI L7 Response Content Compression options are defined here.
//
// Only 2 compression levels are supported; if there is any compression.
// Aggressive Compression: when gzipped is equivalent to level 5
// Normal Compression: is level 1
type CompressionFilterLevel string

const (
	//Aggressive compression will use more Avi CPU, but will provide slightly improved compression percentage
	AGGRESSIVE_COMPRESSION CompressionFilterLevel = "Aggressive"
	//Normal compression level uses some Avi CPU, and provides good compression
	NORMAL_COMPRESSION CompressionFilterLevel = "Normal"
	//No compression
	NO_COMPRESSION CompressionFilterLevel = "None"
)
...
// +kubebuilder:validation:Enum=HTTP_REDIRECT_STATUS_CODE_301;HTTP_REDIRECT_STATUS_CODE_302;HTTP_REDIRECT_STATUS_CODE_307;HTTP_REDIRECT_STATUS_CODE_308
...
// +kubebuilder:validation:Enum=HTTP_LOCAL_RESPONSE_STATUS_CODE_429
...
const (
	// TODO - make enums start at zero to indicate default?
	V4  IpAddrType = "V4"
	DNS IpAddrType = "DNS"
	V6  IpAddrType = "V6"
)
...
type IpAddrRange struct {
	// TODO add begin,end as key if required
...
type IpAddr struct {
	// IP address
	// +kubebuilder:validation:Required
	// IPv4/IPv6 Address format <x.x.x.x>/<x:x::x>";
	// +kubebuilder:default:="IpaddrDefault"
	// TODO add rejex validation
	// // +kubebuilder:validation:Key
	Addr string `json:"addr"`
	// +kubebuilder:validation:Required
	Type IpAddrType `json:"type"`
}

type IpAddrPrefix struct {
	// +kubebuilder:validation:Required
	// TODO define ip_addr, mask as key
	// IPv4/IPv6 Prefix format <x.x.x.x/x>/<x:x::x/x>
...
type CompressionFilter struct {
	...
	// +optional
	// +kubebuilder:default:="IS_IN"
	// Whether to apply Filter when group criteria is matched or not
	Match MatchOperation `json:"match,omitempty"`
	// +optional
	// +kubebuilder:validation:Reference:type=IpAddrGroup
	IpAddrsUuid    string         `json:"ip_addrs_uuid,omitempty"`
	...
	// +kubebuilder:default:="NORMAL_COMPRESSION"
	// +kubebuilder:validation:Required
	Level CompressionFilterLevel `json:"level"`
}

// CompressionProfile defines HTTP compression settings.
type CompressionProfile struct {
	// Knob to turn on/off compression profile
	// +kubebuilder:default:=false
	...
	// +kubebuilder:validation:Required
	// disable compression on the backend server;
	// this will save CPU cycles from being spent on compression.
	// It can be off-loaded to an ADC.
	RemoveAcceptEncodingHeader bool `json:"remove_accept_encoding_header"`
	// +kubebuilder:default:="System-Compressible-Content-Types"
	// Compress only content types listed in this string group. Content types not present in this list are not compressed.
	// +kubebuilder:validation:Required
	// Mime types that will be compressed.
	// text/*
	// application/xml
	// application/javascript
	// points to group uuid
	// +kubebuilder:validation:Reference:type=StringGroup
	// +kubebuilder:default:="System-Compressible-Content-Types"
	CompressibleContentUuid string `json:"compressible_content_uuid"`
	// +kubebuilder:default:="AUTO_COMPRESSION"
	// Compress content automatically or add custom filters to define compressible content and compression levels.
	// +kubebuilder:validation:Required
	// Compress content based on AVI defined hueristics,
	// Compression is done on a need basis taking various inputs into consideration
	// SE status and nature of the content and client can affect compression."
	Type CompressionType `json:"type"`
	// Custom filters used when auto compression is not selected.
	// Specific rules based on Content, when auto compression is off
	// +kubebuilder:validation:XListMap:keys=index
	//TODO add a limitation on number of compression filters from SystemDefaultLimits
	...
	// must be either: 512, 1024, 2048, 4096, 8192, 16384, or 32768
	// +kubebuilder:validation:Enum:=512;1024;2048;4096;8192;16384;32768
	// +optional
	...
	// must be either: 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536,131072
	// +kubebuilder:validation:Enum:=512;1024;2048;4096;8192;16384;32768;65536;131072
	// +optional
	...
	// +kubebuilder:default:="System-Devices-Mobile"
	// Values that identify mobile browsers in order to enable aggressive compression.
	// +optional
	// +kubebuilder:validation:Reference:type=StringGroup
	MobileStrUuid string `json:"mobile_str_uuid,omitempty"`
}
...
type RateLimiterAction struct {
	...
	// +kubebuilder:validation:Enum=200;204;403;404;429;501
