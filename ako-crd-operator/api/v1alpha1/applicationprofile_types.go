...
// +kubebuilder:validation:XValidation:rule="self.xff_update != 'APPEND_TO_THE_XFF_HEADER' || !has(self.xff_alternate_name)",message="if xff_update is APPEND_TO_THE_XFF_HEADER, xff_alternate_name must not be set to a custom value"
