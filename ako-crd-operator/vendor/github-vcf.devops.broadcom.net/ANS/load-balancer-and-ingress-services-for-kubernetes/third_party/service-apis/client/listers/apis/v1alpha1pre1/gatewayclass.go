...
type GatewayClassLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1pre1.GatewayClass, err error)
	// Get retrieves the GatewayClass from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1pre1.GatewayClass, error)
...
// List lists all GatewayClasses in the indexer.
func (s *gatewayClassLister) List(selector labels.Selector) (ret []*v1alpha1pre1.GatewayClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1pre1.GatewayClass))
	})
	return ret, err
}

// Get retrieves the GatewayClass from the index for a given name.
func (s *gatewayClassLister) Get(name string) (*v1alpha1pre1.GatewayClass, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1pre1.Resource("gatewayclass"), name)
	}
	return obj.(*v1alpha1pre1.GatewayClass), nil
}
