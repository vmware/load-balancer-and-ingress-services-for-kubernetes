...
ako/ako              	1.7.4        	    1.7.4     	        A helm chart for Avi Kubernetes Operator
...
```
helm show values ako/ako --version 1.7.4 > values.yaml
...
```
helm install  ako/ako  --generate-name --version 1.7.4 -f /path/to/values.yaml  --set ControllerSettings.controllerHost=<controller IP or Hostname> --set avicredentials.username=<avi-ctrl-username> --set avicredentials.password=<avi-ctrl-password> --set AKOSettings.primaryInstance=true --namespace=avi-system
```

<b>Secondary AKO installation</b>
```
helm install  ako/ako  --generate-name --version 1.7.4 -f /path/to/values.yaml  --set ControllerSettings.controllerHost=<controller IP or Hostname> --set avicredentials.username=<avi-ctrl-username> --set avicredentials.password=<avi-ctrl-password> --set AKOSettings.primaryInstance=false --namespace=avi-system
...
```
helm template ako/ako --version 1.7.4 --include-crds --output-dir <output_dir>
```
...
ako-1593523840	avi-system	1       	2020-09-16 13:44:31.609195757 +0000 UTC	    deployed	ako-1.7.3	1.7.3
```
...
```
helm upgrade ako-1593523840 ako/ako -f /path/to/values.yaml --version 1.7.4 --set ControllerSettings.controllerHost=<IP or Hostname> --set avicredentials.password=<password> --set avicredentials.username=<username> --namespace=avi-system
