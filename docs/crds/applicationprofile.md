...
**NOTE**: ApplicationProfile CRD is specifically designed for use with Avi Load Balancer and is handled by the ako-crd-operator. This CRD supports only HTTP application profile types in the current release.
...
### Using ApplicationProfile with L4Rule

ApplicationProfile can also be referenced in L4Rule for Layer 4 LoadBalancer services:
...
spec:
  backendProperties:
    - port: 8080
      protocol: TCP
      applicationProfile: "basic-http-profile"
```
...
- ApplicationProfile resources can be created in any namespace
- When referenced by HTTPRoute using ExtensionRef, the ApplicationProfile and HTTPRoute must be in the same namespace
- When referenced by L4Rule, both the ApplicationProfile and the referencing resource should typically be in the same namespace
- The ApplicationProfile name must be unique within the namespace
- ApplicationProfile tenant must match the namespace tenant for proper processing
