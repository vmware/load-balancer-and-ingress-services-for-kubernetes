...
**Note**: In **SNI** mode, if GSLB FQDN is specified for an insecure hostname (defined in an Ingress or an OpenShift route), then an httppolicyset object, replacing GSLB FQDN with the local FQDN in the host header, is added to the corresponding parent Shared virtual service to ensure that insecure traffic can be served via the parent Shared VS on the GSLB FQDN. This behaviour also applies to secure hostnames defined in an OpenShift route with **insecureEdgeTerminationPolicy** set to **Allow** in the TLS setting. This is because when insecureEdgeTerminationPolicy is Allow, AKO needs to enable the host to be accessed via HTTP (port 80) to serve insecure traffic via the parent Shared VS. So, if GSLB FQDN is specified, then one httppolicyset is added to the parent shared VS for each such insecure host or secure host with insecureEdgeTerminationPolicy set to Allow. However, a virtual service can have a maximum of sixteen httppolicyset objects associated with it. This implies that we can only have a maximum of sixteen such hostnames associated with a single parent shared virtual service. Hence, we can define GSLB FQDN for a maximum of sixteen above-category hostnames per shared parent VS. So, if a SMALL shard size is used in AKO configuration and AKO creates only one shared parent VS, GSLB FQDN will be configured only for the first sixteen above-category hostnames processed. This limitation will be fixed in a future release.
