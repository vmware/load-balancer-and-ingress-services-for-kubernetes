...
func (t *T1LRNetworking) SyncLSLRNetwork() {
	lslrmap, lrNSMap, cidrs := lib.GetNetworkInfoCRData(lib.GetDynamicClientSet())
	...
	t.createInfraSettingAndAnnotateNS(lrNSMap, cidrs)
}

func (t *T1LRNetworking) createInfraSettingAndAnnotateNS(lrNSMap map[string]string, cidrs map[string]map[string]struct{}) {
	...
	for lr, ns := range lrNSMap {
		...
		go func(lr, ns string) {
			lib.CreateAviInfraSetting(infraSettingName, netName, lr, "")
	...
	for infraSettingName := range oldInfraSettingCRSet {
		...
		wg.Add(1)
		go func(name string) {
			lib.AKOControlConfig().CRDClientset().AkoV1alpha1().AviInfraSettings().Delete(context.TODO(), name, metav1.DeleteOptions{})
			wg.Done()
		}(infraSettingName)
	}
	wg.Wait()
