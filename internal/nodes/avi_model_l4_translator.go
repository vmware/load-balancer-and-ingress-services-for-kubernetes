			...
			if svcObj.Spec.ExternalTrafficPolicy == v1.ServiceExternalTrafficPolicyLocal {
				...
				if lib.AKOControlConfig().GetEndpointSlicesEnabled() {
					...
					for _, epSliceInt := range epSliceIntList {
						...
						if !found {
							utils.AviLog.Warnf("key: %s, msg: no valid endpointslice found that matches the target port of the pool node", key)
							continue
						}
					...
					if !node4Found && !node6Found {
						utils.AviLog.Infof("key: %s, msg: no endpoint found for node %s in endpointslice, skipping addition of pool server", key, node.Name)
						continue
					}
				} else {
					...
					for _, ss := range epObj.Subsets {
						...
						if !port_match {
							utils.AviLog.Warnf("key: %s, msg: no valid endpoint found that matches the target port of the pool node", key)
							continue
						}
						utils.AviLog.Infof("key: %s, msg: found port match for target port %v", key, poolNode.TargetPort.IntValue())
					...
					if !node4Found && !node6Found {
						utils.AviLog.Infof("key: %s, msg: no address found for node %s in endpoint, skipping addition of pool server", key, node.Name)
						continue
					}
				}
			}
