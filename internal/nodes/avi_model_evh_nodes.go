...
func (o *AviObjectGraph) BuildModelGraphForInsecureEVH(routeIgrObj RouteIngressModel, host, infraSettingName, key string, pathsvcmap HostMetada) {
	...
	evhNodeName := lib.GetEvhNodeName(namespace, host, infraSettingName)
...
func (o *AviObjectGraph) BuildModelGraphForSecureEVH(routeIgrObj RouteIngressModel, ingressHostMap SecureHostNameMapProp, hosts []string, tlssetting TlsSettings, ingName, namespace, infraSettingName, host, key string, paths HostMetada) {
	...
	evhNode := vsNode[0].GetEvhNodeForName(lib.GetEvhNodeName(namespace, host, infraSettingName))
	if evhNode == nil {
		evhNode = &AviEvhVsNode{
			Name:         lib.GetEvhNodeName(namespace, host, infraSettingName),
			VHParentName: vsNode[0].Name,
			Tenant:       lib.GetTenant(),
			EVHParent:    false,
			EvhHostName:  host,
...
func (o *AviObjectGraph) DeletePoolForHostnameForEvh(vsName, hostname string, routeIgrObj RouteIngressModel, pathSvc map[string][]string, key, infraSettingName string, removeFqdn, removeRedir, secure bool) {
	...
	evhNodeName := lib.GetEvhNodeName(namespace, hostname, infraSettingName)
