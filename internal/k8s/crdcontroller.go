	...
	if utils.IsGatewayAPIEnabled() {
		...
		appProfileCRDEventHandler := cache.ResourceEventHandlerFuncs{
			...
			DeleteFunc: func(obj interface{}) {
				...
				metadataJSON, found, err := unstructured.NestedStringMap(appProfile.UnstructuredContent(), "metadata")
				if err != nil || !found {
					utils.AviLog.Warnf("ApplicationProfile CRD metadata not found: %+v", err)
					return
				}
				name := metadataJSON["name"]
				namespace := metadataJSON["namespace"]
				statusJSON, found, err := unstructured.NestedStringMap(appProfile.UnstructuredContent(), "status")
				if err != nil || !found {
...
func isObjectProcessed(obj interface{}, namespace, name string) (bool, string) {
	oldObj := obj.(*unstructured.Unstructured)
	statusJSON, found, err := unstructured.NestedStringMap(oldObj.UnstructuredContent(), "status")
	if err != nil || !found {
		utils.AviLog.Warnf("key:%s/%s, msg:ApplicationProfile CRD status not found: %+v", namespace, name, err)
		return false, ""
	}
...
func updateAppProfileCRDToL7RuleMapping(oldObj, newObj *akov1alpha2.L7Rule) {
	...
	if oldObjSpec.ApplicationProfile != nil {
		...
		if *oldObjSpec.ApplicationProfile.Kind == lib.AVI_CRD {
