...
func AnnotateSystemNamespaceWithInfraSetting() {
	...
	for _, namespace := range systemNamespaces {
		if val, ok := namespace.Annotations[InfraSettingNameAnnotation]; ok && val == vpcUUID {
			continue
		}
		...
		utils.AviLog.Infof("Annotated Namespace %s with AviInfraSetting %s", namespace.GetName(), vpcUUID)
	}
}
