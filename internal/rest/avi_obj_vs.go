	...
	for _, resp := range resp_elems {
		...
		if found_parent {
			...
			if foundvscache {
				...
				utils.AviLog.InfoLog(spew.Sprintf("key: %s, msg: added VS cache key during SNI update %v val %v", key, parentKey,
					vs_cache_obj))
			}
		}
...
func (rest *RestOperations) AviVSDel(uuid string, tenant string, key string) (*utils.RestOp, bool) {
	...
	utils.AviLog.InfoLog(spew.Sprintf("key: %s, msg: VirtualService DELETE Restop %v ",
