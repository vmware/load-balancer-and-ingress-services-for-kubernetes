...
func (rest *RestOperations) ExecuteRestAndPopulateCache(rest_ops []*utils.RestOp, aviObjKey avicache.NamespaceName, avimodel *nodes.AviObjectGraph, key string, isEvh bool, sslKey ...utils.NamespaceName) (bool, bool) {
	...
	if len(aviRestPoolClient.AviClient) > 0 && len(rest_ops) > 0 {
		...
		err := rest.AviRestOperateWrapper(rest_ops, key, bkt)
		if err == nil {
			...
			for i := len(rest_ops) - 1; i >= 0; i-- {
				...
				if rest_ops[i].Err != nil {
					...
					if refreshCacheForRetry {
						...
						aviClient, ok := tenantToAviClient[rest_ops[i].Tenant]
						if !ok || len(aviClient) < 1 {
							aviClient = avicache.SharedAVIClients(rest_ops[i].Tenant).AviClient
						}
						aviclient := aviClient[bkt]
...
func (rest *RestOperations) AviRestOperateWrapper(rest_ops []*utils.RestOp, key string, bkt uint32) error {
	restTimeoutChan := make(chan error, 1)
	go func() {
		err := rest.restOperator.AviRestOperate(rest_ops, key, bkt)
		restTimeoutChan <- err
	}()
	select {
