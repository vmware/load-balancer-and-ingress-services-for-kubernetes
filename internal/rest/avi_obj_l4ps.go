...
func (rest *RestOperations) AviL4PolicyCacheAdd(rest_op *utils.RestOp, vsKey avicache.NamespaceName, key string) error {
	...
	for _, resp := range resp_elems {
		...
		if ok {
			...
			vs_cache_obj.AddToL4PolicyCollection(k)
			utils.AviLog.InfoLog(spew.Sprintf("Added VS cache key during l4 policy update %v val %v", vsKey,
				vs_cache_obj))
		}
		utils.AviLog.InfoLog(spew.Sprintf("Added L4 Policy Set cache k %v val %v", k,
			l4_cache_obj))
	}
