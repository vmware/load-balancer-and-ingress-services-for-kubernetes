...
func SharedAVIClients(tenant string) *utils.AviRestClientPool {
	...
	if ctrlUsername == "" || (ctrlPassword == "" && ctrlAuthToken == "") || ctrlIpAddress == "" {
		...
		if ctrlIpAddress == "" {
			utils.AviLog.Fatalf("Avi Controller information missing (username: %s, password: %s, authToken: %s, controller: %s). Update the controller IP in installation configuration", ctrlUsername, passwordLog, authTokenLog, ctrlIpAddress)
		}
