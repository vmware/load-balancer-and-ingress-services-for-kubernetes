...
### Fixed
 ...
 - Fix: Dedicated insecure VS will now correctly get only port `80` by default if no listeners are defined via any CRD.
 - Fix: When HostRule `tcpSettings` section is defined only with `loadBalancerIP` and no `listeners`, VS creation will not error out; VS will come up successfully with default ports.
