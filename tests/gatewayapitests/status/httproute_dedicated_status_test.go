...
func setupDedicatedGateway(t *testing.T, name, namespace, gatewayClass string, listeners []gatewayv1.Listener) {
	// First create regular gateway
	akogatewayapitests.SetupGateway(t, name, namespace, gatewayClass, nil, listeners)

	// Then add dedicated mode annotation
	gateway, err := akogatewayapitests.GatewayClient.GatewayV1().Gateways(namespace).Get(context.TODO(), name, metav1.GetOptions{})
	if err != nil {
	...
	gateway.Annotations[akogatewayapilib.DedicatedGatewayModeAnnotation] = "true"

	_, err = akogatewayapitests.GatewayClient.GatewayV1().Gateways(namespace).Update(context.TODO(), gateway, metav1.UpdateOptions{})
	if err != nil {
		t.Fatalf("Failed to update gateway with dedicated mode annotation: %v", err)
	}
}
...
func TestHTTPRouteWithMultipleParentRefsInDedicatedMode(t *testing.T) {
	...
	setupDedicatedGateway(t, gatewayName2, namespace, gatewayClassName, listeners)
