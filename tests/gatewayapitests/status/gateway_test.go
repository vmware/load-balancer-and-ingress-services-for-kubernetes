...
// TestGatewayWithHTTPSListenerWithoutTLSInVCFCluster tests that an HTTPS listener without TLS configuration is rejected in VCF cluster.
func TestGatewayWithHTTPSListenerWithoutTLSInVCFCluster(t *testing.T) {
	gatewayClassName := "gateway-class-https-no-tls"
	gatewayName := "gateway-https-no-tls"
	namespace := "default"

	tests.SetupGatewayClass(t, gatewayClassName, akogatewayapilib.GatewayController)

	os.Setenv("VCF_CLUSTER", "true")
	defer os.Unsetenv("VCF_CLUSTER")

	ports := []int32{443}
	listeners := tests.GetListenersV1(ports, false, false)

	tests.SetupGateway(t, gatewayName, namespace, gatewayClassName, nil, listeners)

	g := gomega.NewGomegaWithT(t)

	// Wait for Gateway listener status to be updated with rejection
	g.Eventually(func() bool {
		gateway, err := tests.GatewayClient.GatewayV1().Gateways(namespace).Get(context.TODO(), gatewayName, metav1.GetOptions{})
		if err != nil || gateway == nil {
			t.Logf("Couldn't get the gateway, err: %+v", err)
			return false
		}
		...
		acceptedCondition := apimeta.FindStatusCondition(gateway.Status.Listeners[0].Conditions, string(gatewayv1.ListenerConditionAccepted))
		if acceptedCondition == nil {
			return false
		}
		// Check if the listener is rejected due to missing TLS config
		return acceptedCondition.Status == metav1.ConditionFalse &&
			strings.Contains(acceptedCondition.Message, "TLS is not configured for HTTPS listener")
	}, 30*time.Second).Should(gomega.Equal(true))

	// Cleanup
	tests.TeardownGateway(t, gatewayName, namespace)
	tests.TeardownGatewayClass(t, gatewayClassName)
}

// @AI-Generated
// [Generated by Cursor claude-4-sonnet]
// TestGatewayWithNonTLSSecretInVCFCluster tests that a secret that is not of type kubernetes.io/tls is rejected in VCF cluster	.
func TestGatewayWithNonTLSSecretInVCFCluster(t *testing.T) {
	gatewayClassName := "gateway-class-non-tls-secret"
	gatewayName := "gateway-non-tls-secret"
	namespace := "default"
	secretName := "opaque-secret"

	tests.SetupGatewayClass(t, gatewayClassName, akogatewayapilib.GatewayController)

	integrationtest.AddSecret(secretName, namespace, "cert", "key")

	os.Setenv("VCF_CLUSTER", "true")
	defer os.Unsetenv("VCF_CLUSTER")

	ports := []int32{443}
	listeners := tests.GetListenersV1(ports, false, false, secretName)

	tests.SetupGateway(t, gatewayName, namespace, gatewayClassName, nil, listeners)

	g := gomega.NewGomegaWithT(t)

	// Wait for Gateway listener status to be updated with rejection due to non-TLS secret
	g.Eventually(func() bool {
		gateway, err := tests.GatewayClient.GatewayV1().Gateways(namespace).Get(context.TODO(), gatewayName, metav1.GetOptions{})
		if err != nil || gateway == nil {
			t.Logf("Couldn't get the gateway, err: %+v", err)
		...
		resolvedRefsCondition := apimeta.FindStatusCondition(gateway.Status.Listeners[0].Conditions, string(gatewayv1.ListenerConditionResolvedRefs))
		if resolvedRefsCondition == nil {
			return false
		}
		// Check if the listener is rejected due to non-TLS secret type
		return resolvedRefsCondition.Status == metav1.ConditionFalse &&
			strings.Contains(resolvedRefsCondition.Message, "Secret is not of type TLS")
	}, 30*time.Second).Should(gomega.Equal(true))

	// Cleanup
	tests.TeardownGateway(t, gatewayName, namespace)
	tests.TeardownGatewayClass(t, gatewayClassName)
	integrationtest.DeleteSecret(secretName, namespace)
}

// @AI-Generated
// [Generated by Cursor claude-4-sonnet]
// TestGatewayWithValidTLSSecret tests that a valid TLS secret is accepted.
func TestGatewayWithValidTLSSecret(t *testing.T) {
	gatewayClassName := "gateway-class-valid-tls-secret"
	gatewayName := "gateway-valid-tls-secret"
	namespace := "default"
	secretName := "valid-tls-secret"

	tests.SetupGatewayClass(t, gatewayClassName, akogatewayapilib.GatewayController)

	// Create a proper TLS secret
	integrationtest.AddTLSSecret(secretName, namespace, "cert-data", "key-data")
	...
	// Wait for Gateway listener status to be accepted with valid TLS secret
	g.Eventually(func() bool {
		gateway, err := tests.GatewayClient.GatewayV1().Gateways(namespace).Get(context.TODO(), gatewayName, metav1.GetOptions{})
		if err != nil || gateway == nil {
			t.Logf("Couldn't get the gateway, err: %+v", err)
		...
		// Check if the listener has resolved refs successfully
		return resolvedRefsCondition.Status == metav1.ConditionTrue
	}, 30*time.Second).Should(gomega.Equal(true))
...
// @AI-Generated
// [Generated by Cursor claude-4-sonnet]
// TestGatewayStatusWithParentVSRESTError tests that Gateway Programmed condition is updated with error message
// when a REST operation fails for the parent VS (VS_TYPE_VH_PARENT).
// This validates the error propagation from updateGatewayStatusWithVsError for parent VS.
func TestGatewayStatusWithParentVSRESTError(t *testing.T) {
	gatewayClassName := "gateway-class-rest-error"
	gatewayName := "gateway-rest-error"
	namespace := "default"
	ports := []int32{8080}
	...
	// Now inject middleware to fail POST calls to /api/virtualservice when VS type is VS_TYPE_VH_PARENT
	parentVSErrorMessage := "Gateway realization failed due to test error"
	integrationtest.AddMiddleware(func(w http.ResponseWriter, r *http.Request) {
		url := r.URL.EscapedPath()

		if r.Method == http.MethodPost && strings.Contains(url, "/api/virtualservice") {
			w.WriteHeader(http.StatusBadRequest)
			errorResponse := map[string]interface{}{
				"error": parentVSErrorMessage,
			}
		...
		// For all other requests, use normal controller server
		integrationtest.NormalControllerServer(w, r, "../../avimockobjectsgw")
	})
	defer integrationtest.ResetMiddleware()

	// Wait for Gateway listener Programmed condition to be updated with error message
	// The Programmed condition on the listener should be False with a message containing the error
	g.Eventually(func() bool {
		gateway, err := tests.GatewayClient.GatewayV1().Gateways(namespace).Get(context.TODO(), gatewayName, metav1.GetOptions{})
		if err != nil || gateway == nil {
			t.Logf("Couldn't get the gateway, err: %+v", err)
		...
		programmedCondition := apimeta.FindStatusCondition(gateway.Status.Listeners[0].Conditions, string(gatewayv1.ListenerConditionProgrammed))
		if programmedCondition == nil {
			return false
		}
		// Check if the condition has the error message (error from REST call)
		return programmedCondition.Status == metav1.ConditionFalse &&
			strings.Contains(programmedCondition.Message, parentVSErrorMessage)
	}, 40*time.Second).Should(gomega.Equal(true))
