...
import (
	...
	"github.com/vmware/load-balancer-and-ingress-services-for-kubernetes/ako-gateway-api/nodes"
...
func TestHTTPRouteWithValidConfig(t *testing.T) {
	...
	g.Eventually(func() bool {
		...
		return akogatewayapitests.ValidateHTTPRouteStatusWithRetry(t, &httpRoute.Status, &gatewayv1.HTTPRouteStatus{RouteStatus: *expectedRouteStatus})
	}, 30*time.Second).Should(gomega.Equal(true))
...
func TestHTTPRouteWithInvalidBackendKind(t *testing.T) {
	...
	rule := akogatewayapitests.GetHTTPRouteRuleV1(integrationtest.PATHPREFIX, []string{"/foo"}, []string{},
		map[string][]string{"RequestHeaderModifier": {"add", "remove", "replace"}},
		[][]string{{"avisvc", namespace, "8080", "1"}}, nil)
...
func verifyHTTPRouteStatus(t *testing.T, g *gomega.GomegaWithT, httpRouteName, gatewayName, namespace string, ports []int32, condition *metav1.Condition) {
	...
	g.Eventually(func() bool {
		...
		return akogatewayapitests.ValidateHTTPRouteStatusWithRetry(t, &hr.Status, &gatewayv1.HTTPRouteStatus{RouteStatus: *expectedRouteStatus})
	}, 30*time.Second).Should(gomega.Equal(true))
}
...
// @AI-Generated
// [Generated by Cursor claude-4-sonnet]
// TestHTTPRouteStatusWithChildVSRESTError tests that HTTPRoute status is updated with error message
// when a REST operation fails for a child VS (VS_TYPE_VH_CHILD).
// This validates the new updateHTTPRouteStatusWithErrorMessage function and the error propagation
// from updateGatewayStatusWithVsError for child VS.
func TestHTTPRouteStatusWithChildVSRESTError(t *testing.T) {
	gatewayClassName := "gateway-class-rest-error"
	gatewayName := "gateway-rest-error"
	httpRouteName := "httproute-rest-error"
	namespace := "default"
	svcName := "avisvc-rest-error"
	ports := []int32{8080}
	...
	g := gomega.NewGomegaWithT(t)
	// Wait for Gateway to be programmed (parent VS should succeed)
	g.Eventually(func() bool {
		gateway, err := akogatewayapitests.GatewayClient.GatewayV1().Gateways(namespace).Get(context.TODO(), gatewayName, metav1.GetOptions{})
		if err != nil || gateway == nil {
			t.Logf("Couldn't get the gateway, err: %+v", err)
		...
		return apimeta.FindStatusCondition(gateway.Status.Conditions, string(gatewayv1.GatewayConditionProgrammed)) != nil &&
			apimeta.FindStatusCondition(gateway.Status.Conditions, string(gatewayv1.GatewayConditionProgrammed)).Status == metav1.ConditionTrue
	}, 30*time.Second).Should(gomega.Equal(true))

	// Now inject middleware to fail POST calls to /api/virtualservice when VS type is VS_TYPE_VH_CHILD
	// We do this AFTER the gateway is programmed so parent VS creation succeeds
	childVSErrorMessage := "Child VS creation failed due to test error"
	integrationtest.AddMiddleware(func(w http.ResponseWriter, r *http.Request) {
		url := r.URL.EscapedPath()

		// Return error for child VS, Gateway is programmed so we can safely assume that
		// POST to /api/virtualservice is for child VS
		if r.Method == http.MethodPost && strings.Contains(url, "/api/virtualservice") {
			w.WriteHeader(http.StatusBadRequest)
			errorResponse := map[string]interface{}{
				"error": childVSErrorMessage,
			}
	...
	// Wait for HTTPRoute status to be updated with error message
	// The Accepted condition should remain True but have a message starting with "ProgrammingFailed"
	g.Eventually(func() bool {
		httpRoute, err := akogatewayapitests.GatewayClient.GatewayV1().HTTPRoutes(namespace).Get(context.TODO(), httpRouteName, metav1.GetOptions{})
		if err != nil || httpRoute == nil {
			t.Logf("Couldn't get the HTTPRoute, err: %+v", err)
		...
		// Check if the condition has the error message
		return acceptedCondition.Status == metav1.ConditionTrue &&
			strings.HasPrefix(acceptedCondition.Message, "ProgrammingFailed:") &&
			strings.Contains(acceptedCondition.Message, childVSErrorMessage)
