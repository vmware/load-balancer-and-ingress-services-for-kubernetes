...
func DeleteRouteBackendExtensionCR(t *testing.T, name, namespace string) {
	err := DynamicClient.Resource(akogatewayapilib.RouteBackendExtensionCRDGVR).Namespace(namespace).Delete(context.TODO(), name, metav1.DeleteOptions{})
	if err != nil {
		t.Fatalf("error in deleting RouteBackendExtension: %v", err)
	}
	t.Logf("Deleted RouteBackendExtension %s/%s", namespace, name)
}
