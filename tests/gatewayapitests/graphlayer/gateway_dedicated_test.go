...
func GetDedicatedListenersV1(ports []int32, secrets ...string) []gatewayv1.Listener {
	...
	for _, port := range ports {
		...
		if len(secrets) > 0 {
			...
			listener.Protocol = gatewayv1.ProtocolType("HTTPS") // Change to HTTPS for TLS
		}
...
// Helper function to set up a gateway with dedicated mode annotation
...
// Helper function to update a gateway with dedicated mode annotation
...
func TestGatewayRegularToDedicatedMode(t *testing.T) {
	...
	UpdateDedicatedGateway(t, gatewayName, DEFAULT_NAMESPACE, gatewayClassName, nil, listeners)
