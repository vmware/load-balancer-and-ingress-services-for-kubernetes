...
func validateHTTPRouteWithInfraSetting(g *gomega.GomegaWithT, tenant, gatewayName, infraSettingName string) {
	...
	validateChildNode := func(childNode *avinodes.AviEvhVsNode) {
		...
		g.Expect(len(childNode.PoolRefs[0].NetworkPlacementSettings)).To(gomega.Equal(1))
		g.Expect(func() bool {
			_, ok := childNode.PoolRefs[0].NetworkPlacementSettings["thisisaviref-"+infraSettingName+"-nodeNetworkName"]
			return ok
...
func validateHTTPRouteWithoutInfraSetting(g *gomega.GomegaWithT, tenant, gatewayName string) {
	...
	validateChildNode := func(childNode *avinodes.AviEvhVsNode) {
		...
		g.Expect(len(childNode1.PoolRefs[0].NetworkPlacementSettings)).To(gomega.Equal(0))
	}
...
func TestHTTPRouteWithInfraSetting(t *testing.T) {
	...
	tests.SetupGatewayClassWithInfraSetting(t, gatewayClassName, akogatewayapilib.GatewayController, infraSettingName)
...
func TestHTTPRouteCreateInfraSetting(t *testing.T) {
	...
	g.Eventually(func() bool {
		...
		nodes := aviModel.(*avinodes.AviObjectGraph).GetAviEvhVS()
		childNode1 := nodes[0].EvhNodes[0]
		childNode2 := nodes[0].EvhNodes[0]
		_, ok1 := childNode1.PoolRefs[0].NetworkPlacementSettings["thisisaviref-"+infraSettingName+"-nodeNetworkName"]
		_, ok2 := childNode2.PoolRefs[0].NetworkPlacementSettings["thisisaviref-"+infraSettingName+"-nodeNetworkName"]
		return ok1 && ok2 &&
			childNode1.PoolRefs[0].T1Lr == "avi-domain-c9:1234" &&
			childNode2.PoolRefs[0].T1Lr == "avi-domain-c9:1234"
	}, 25*time.Second).Should(gomega.Equal(true))
...
func TestHTTPRouteUpdateInfraSettingStatusToRejected(t *testing.T) {
	...
	g.Eventually(func() bool {
		...
		childNode1 := nodes[0].EvhNodes[0]
		childNode2 := nodes[0].EvhNodes[0]
		return len(childNode1.PoolRefs[0].NetworkPlacementSettings) == 0 &&
			len(childNode2.PoolRefs[0].NetworkPlacementSettings) == 0 &&
			childNode1.PoolRefs[0].T1Lr == "test-t1lr" &&
			childNode2.PoolRefs[0].T1Lr == "test-t1lr"
	}, 25*time.Second).Should(gomega.Equal(true))
