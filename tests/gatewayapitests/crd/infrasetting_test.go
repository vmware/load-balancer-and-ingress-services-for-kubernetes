...
func TestGatewayWithInfraSetting(t *testing.T) {
	...
	g.Expect(nodes[0].VSVIPRefs[0].VipNetworks[0].NetworkName).Should(gomega.Equal("thisisaviref-" + infraSettingName + "-networkName"))
...
func TestGatewayCreateInfraSetting(t *testing.T) {
	...
	g.Eventually(func() bool {
		...
		return nodes[0].ServiceEngineGroup == "thisisaviref-"+infraSettingName+"-seGroup" &&
			...
			nodes[0].VSVIPRefs[0].VipNetworks[0].NetworkName == "thisisaviref-"+infraSettingName+"-networkName"
	}, 25*time.Second).Should(gomega.Equal(true))
