...
func TestHTTPRouteWithHealthMonitor(t *testing.T) {
	gatewayClassName := "gateway-class-hm-simple"
	gatewayName := "gateway-hm-simple"
	httpRouteName := "httproute-hm-simple"
	healthMonitorName := "hm-simple"
	namespace := "default"
	ports := []int32{8080}
	key := "HTTPRoute" + "/" + namespace + "/" + httpRouteName
	gwKey := "Gateway/" + DEFAULT_NAMESPACE + "/" + gatewayName
	...
	akogatewayapitests.CreateHealthMonitorCRD(t, healthMonitorName, DEFAULT_NAMESPACE, "thisisaviref-hm-simple")

	// Setup HTTPRoute with HealthMonitor
	parentRefs := akogatewayapitests.GetParentReferencesV1([]string{gatewayName}, namespace, ports)
	hostnames := []gatewayv1.Hostname{"simple-8080.com"}
	rule := akogatewayapitests.GetHTTPRouteRuleWithHealthMonitorFilters(integrationtest.PATHPREFIX, []string{"/simple"}, []string{},
		map[string][]string{"RequestHeaderModifier": {"add"}},
		[][]string{{"avisvc", "default", "8080", "1"}}, []string{healthMonitorName})
	rules := []gatewayv1.HTTPRouteRule{rule}
	akogatewayapitests.SetupHTTPRoute(t, httpRouteName, namespace, parentRefs, hostnames, rules)
	t.Logf("Created HTTPRoute %s with HealthMonitor %s", httpRouteName, healthMonitorName)
	waitAndverify(t, key)

	// update httproute to healthmonitor cache
	akogatewayapiobjects.GatewayApiLister().UpdateHTTPRouteToHealthMonitorMapping(namespace+"/"+httpRouteName, namespace+"/"+healthMonitorName)
	akogatewayapiobjects.GatewayApiLister().UpdateHealthMonitorToHTTPRoutesMapping(namespace+"/"+healthMonitorName, namespace+"/"+httpRouteName)

	// update healthmonitor status to false
	...
	// Delete Gateway
	akogatewayapitests.TeardownGateway(t, gatewayName, namespace)
	t.Logf("Deleted Gateway %s", gatewayName)
	waitAndverify(t, gwKey)

	// Delete GatewayClass
	akogatewayapitests.TeardownGatewayClass(t, gatewayClassName)
	t.Logf("Deleted GatewayClass %s", gatewayClassName)
	waitAndverify(t, gwClassKey)
}
