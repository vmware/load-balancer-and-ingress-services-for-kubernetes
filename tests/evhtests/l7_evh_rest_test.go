...
func TestCreateIngressCacheSyncForEvh(t *testing.T) {
	...
	sniVSKey := cache.NamespaceName{Namespace: "admin", Name: lib.Encode("cluster--default-foo.com", "SNI VS ")}
...
func TestUpdatePoolCacheSyncForEvh(t *testing.T) {
	...
	mcache := cache.SharedAviObjCache()
	poolKey := cache.NamespaceName{Namespace: integrationtest.AVINAMESPACE, Name: lib.Encode(poolName, "Pool ")}
...
func TestUpdateCacheSyncForEvh(t *testing.T) {
	...
	oldHttpPolKey := cache.NamespaceName{Namespace: "admin", Name: lib.Encode("cluster--default-foo.com_foo-foo-with-targets", "HTTP Policyset")}
	newHttpPolKey := cache.NamespaceName{Namespace: "admin", Name: lib.Encode("cluster--default-foo.com_bar-updated-foo-with-targets", "HTTP Policyset ")}
	g.Eventually(func() bool {
...
func TestMultiHostMultiSecretCacheSyncForEvh(t *testing.T) {
	...
	sniVSKey1 := cache.NamespaceName{Namespace: "admin", Name: lib.Encode("cluster--default-foo.com", "SNI VS ")}
	sniVSKey2 := cache.NamespaceName{Namespace: "admin", Name: lib.Encode("cluster--default-bar.com", "SNI VS ")}
	g.Eventually(func() bool {
	...
	g.Eventually(func() string {
		...
		return ""
	}, 40*time.Second).Should(gomega.Equal(lib.Encode("cluster--foo.com", "SSL Key ")))
	...
	g.Eventually(func() string {
		...
		return ""
	}, 40*time.Second).Should(gomega.Equal(lib.Encode("cluster--bar.com", "SSL Key ")))
	...
	sniVSKey3 := cache.NamespaceName{Namespace: "admin", Name: lib.Encode("cluster--red-foo.com", "SNI VS ")}
	g.Eventually(func() bool {
...
func TestMultiHostMultiSecretUpdateCacheSyncForEvh(t *testing.T) {
	...
	g.Expect(parentCacheObj.SSLKeyCertCollection).To(gomega.HaveLen(1))
	g.Expect(parentCacheObj.SSLKeyCertCollection[0].Name).To(gomega.Equal(lib.Encode("cluster--foo.com", "SSL Key ")))

	g.Expect(barCacheObj.SSLKeyCertCollection).To(gomega.HaveLen(1))
	g.Expect(barCacheObj.SSLKeyCertCollection[0].Name).To(gomega.Equal(lib.Encode("cluster--bar.com", "SSL Key ")))
...
func TestCUDSecretCacheSyncForEvh(t *testing.T) {
	...
	parentVSKey := cache.NamespaceName{Namespace: "admin", Name: "cluster--Shared-L7-EVH-0"}
	sniVSKey := cache.NamespaceName{Namespace: "admin", Name: lib.Encode("cluster--default-foo.com", "SNI VS ")}
	sslKey := cache.NamespaceName{Namespace: "admin", Name: lib.Encode("cluster--foo.com", "SNI VS ")}
