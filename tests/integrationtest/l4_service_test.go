...
func SetUpTestForSvcLB(t *testing.T, svcName, svcModel string) {
	objects.SharedAviGraphLister().Delete(svcName)
	...
	PollForCompletion(t, svcModel, 5)
}

func SetUpTestForSvcLBWithLBClass(t *testing.T, LBClass string, svcName, svcModel string) {
...
func TearDownTestForSvcLB(t *testing.T, g *gomega.GomegaWithT, svcName, svcModel string) {
	objects.SharedAviGraphLister().Delete(svcModel)
...
func TestLBSvcWithLBClassWithAviDefaultLBController(t *testing.T) {
	...
	svcName := SINGLEPORTSVC + "-22"
	svcModel := SINGLEPORTMODEL + "-22"

	// test invalid service spec.LoadBalancerClass != ako.vmware.com/avi-lb
	SetUpTestForSvcLBWithLBClass(t, INVALID_LB_CLASS, svcName, svcModel)
	g.Eventually(func() bool {
	...
	SetUpTestForSvcLBWithLBClass(t, "ako.vmware.com/avi-lb", svcName, svcModel)
	g.Eventually(func() bool {
		found, _ := objects.SharedAviGraphLister().Get(svcModel)
		return found
...
func TestLBSvcWithLBClassWithoutAviDefaultLBController(t *testing.T) {
	...
	svcName := SINGLEPORTSVC + "-23"
	svcModel := SINGLEPORTMODEL + "-23"
...
func TestAviSvcCreationSinglePort(t *testing.T) {
	...
	svcName := SINGLEPORTSVC + "-24"
	svcModel := SINGLEPORTMODEL + "-24"
	SetUpTestForSvcLB(t, svcName, svcModel)
...
func TestAviSvcCreationSinglePortMultiTenantEnabled(t *testing.T) {
	...
	svcName := SINGLEPORTSVC + "-25"
	modelName := fmt.Sprintf("%s/cluster--red-ns-"+svcName, AKOTENANT)
...
func TestAviSvcCreationMultiPort(t *testing.T) {
	g := gomega.NewGomegaWithT(t)
	svcName := MULTIPORTSVC + "-26"

	modelName := fmt.Sprintf("%s/cluster--%s-%s", AVINAMESPACE, NAMESPACE, svcName)

	SetUpTestForSvcLBMultiport(t, svcName, modelName)
...
func TestAviSvcUpdateEndpoint(t *testing.T) {
	g := gomega.NewGomegaWithT(t)
	svcName := SINGLEPORTSVC + "-29"
	modelName := fmt.Sprintf("%s/cluster--%s-%s", AVINAMESPACE, NAMESPACE, svcName)

	SetUpTestForSvcLB(t, svcName, modelName)
	...
	TearDownTestForSvcLB(t, g, svcName, modelName)
}
...
func TestCreateServiceLBCacheSync(t *testing.T) {
	...
	svcName := SINGLEPORTSVC + "-30"
	svcModel := SINGLEPORTMODEL + "-30"
...
func TestCreateServiceLBWithFaultCacheSync(t *testing.T) {
	...
	svcName := SINGLEPORTSVC + "-31"
	svcModel := SINGLEPORTMODEL + "-31"
...
func TestCreateMultiportServiceLBCacheSync(t *testing.T) {
	g := gomega.NewGomegaWithT(t)
	svcName := MULTIPORTSVC + "-32"
	svcModel := MULTIPORTMODEL + "-32"
...
func TestScaleUpAndDownServiceLBCacheSync(t *testing.T) {
	...
	for i := 0; i < numScale; i++ {
		service = fmt.Sprintf("%s-%d", MULTIPORTSVC, i+35)
...
func TestAviSvcCreationWithStaticIP(t *testing.T) {
	...
	svcName := SINGLEPORTSVC + "-40"
	svcModel := SINGLEPORTMODEL + "-40"
	...
	g.Eventually(func() string {
		if found, aviModel := objects.SharedAviGraphLister().Get(svcModel); found && aviModel != nil {
			nodes := aviModel.(*avinodes.AviObjectGraph).GetAviVS()
...
func TestWithInfraSettingStatusUpdates(t *testing.T) {
	...
	g := gomega.NewGomegaWithT(t)
	svcName := SINGLEPORTSVC + "-41"
	svcModel := SINGLEPORTMODEL + "-41"
	settingName := "infra-setting-1"
...
func TestInfraSettingDelete(t *testing.T) {
	...
	g := gomega.NewGomegaWithT(t)
	svcName := SINGLEPORTSVC + "-42"
	svcModel := SINGLEPORTMODEL + "-42"
	settingName := "infra-setting-2"
...
func TestInfraSettingChangeMapping(t *testing.T) {
	...
	g := gomega.NewGomegaWithT(t)
	svcName := SINGLEPORTSVC + "-43"
	svcModel := SINGLEPORTMODEL + "-43"

	settingName1, settingName2 := "infra-setting-3", "infra-setting-4"
...
func TestLBSvcCreationMixedProtocol(t *testing.T) {
	g := gomega.NewGomegaWithT(t)
	svcName := SINGLEPORTSVC + "-44"
	svcModel := SINGLEPORTMODEL + "-44"
	SetUpTestForSvcLBMixedProtocol(t, svcName, svcModel, corev1.ProtocolTCP, corev1.ProtocolUDP, corev1.ProtocolSCTP)
...
func TestLBSvcCreationSCTPTCP(t *testing.T) {
	g := gomega.NewGomegaWithT(t)
	svcName := SINGLEPORTSVC + "-45"
	svcModel := SINGLEPORTMODEL + "-45"
	...
	SetUpTestForSvcLBMixedProtocol(t, svcName, svcModel, corev1.ProtocolTCP, corev1.ProtocolSCTP)
...
func TestLBSvcCreationSCTPUDP(t *testing.T) {
	g := gomega.NewGomegaWithT(t)
	svcName := SINGLEPORTSVC + "-46"
	svcModel := SINGLEPORTMODEL + "-46"
	...
	SetUpTestForSvcLBMixedProtocol(t, svcName, svcModel, corev1.ProtocolUDP, corev1.ProtocolSCTP)
...
func TestLBSvcCreationTCPUDP(t *testing.T) {
	g := gomega.NewGomegaWithT(t)
	svcName := SINGLEPORTSVC + "-47"
	svcModel := SINGLEPORTMODEL + "-47"
	...
	SetUpTestForSvcLBMixedProtocol(t, svcName, svcModel, corev1.ProtocolTCP, corev1.ProtocolUDP)
