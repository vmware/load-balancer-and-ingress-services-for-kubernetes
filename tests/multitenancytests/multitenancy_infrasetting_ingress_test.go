...
// Description: UT to test update in namespace annotation for PassThrough Ingress
func TestMultiTenancyWithTenantDeannotationInNSForPassThroughIngress(t *testing.T) {
	// create an ingress, infrasetting and annotate a namespace with tenant
	// graph layer objects should come up with correct tenant
	// delete the tenant annotation from the namespace, old model should be deleted
	// new model in default tenant should get created
	g := gomega.NewGomegaWithT(t)
	...
	ns := "default"
	settingName := objNameMap.GenerateName("my-infrasetting")
	modelName := "nonadmin/cluster--Shared-Passthrough-0" // Correct initial model name
	svcName := objNameMap.GenerateName("avisvc")
	time.Sleep(time.Second * 5)
	ingestionQueue := utils.SharedWorkQueue().GetQueueByName(utils.ObjectIngestionLayer)
	ingestionQueue.SyncFunc = syncFromIngestionLayerWrapper
	...
	ingressCreate.SetAnnotations(ann)
	_, err := KubeClient.NetworkingV1().Ingresses(ns).Create(context.TODO(), ingressCreate, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("error in adding Ingress: %v", err)
	}

	// Verify initial creation in nonadmin tenant
	aviModel := ValidatePassthroughModel(t, g, modelName)
	graph := aviModel.(*avinodes.AviObjectGraph)
	vs := graph.GetAviVS()[0]
	VerifyPassthrough(t, g, vs, svcName)
	g.Expect(vs.Tenant).To(gomega.Equal("nonadmin"))

	integrationtest.RemoveAnnotateAKONamespaceWithInfraSetting(t, ns)
	VerifyPassthroughIngressDeletion(t, g, modelName, 0, 0)

	defaultModelName := "admin/cluster--Shared-Passthrough-0"

	// Verify creation in default (admin) tenant
	aviModel = ValidatePassthroughModel(t, g, defaultModelName)
	graph = aviModel.(*avinodes.AviObjectGraph)
	vs = graph.GetAviVS()[0]
	VerifyPassthrough(t, g, vs, svcName)
	g.Expect(vs.Tenant).To(gomega.Equal("admin"))

	err = KubeClient.NetworkingV1().Ingresses(ns).Delete(context.TODO(), ingressName, metav1.DeleteOptions{})
	if err != nil {
		t.Fatalf("Couldn't DELETE the Ingress %v", err)
	}
	VerifyPassthroughIngressDeletion(t, g, defaultModelName, 0, 0)
	ingresstests.TearDownTestForIngress(t, svcName, defaultModelName)
	integrationtest.TeardownIngressClass(t, ingClassName)
	waitAndVerify(t, ingClassName)
	integrationtest.TeardownAviInfraSetting(t, settingName)
}
...
func ValidatePassthroughModel(t *testing.T, g *gomega.WithT, modelName string) interface{} {
	g.Eventually(func() interface{} {
		...
		vsNodes := aviModel.(*avinodes.AviObjectGraph).GetAviVS()
		if len(vsNodes) == 0 {
			return nil
		}
		return aviModel
	}, 55*time.Second, time.Second).ShouldNot(gomega.BeNil(), "VS not found for model %s", modelName)

	found, aviModel := objects.SharedAviGraphLister().Get(modelName)
	g.Expect(found).To(gomega.BeTrue(), "Model %s not found", modelName)
...
func VerifyPassthrough(t *testing.T, g *gomega.WithT, vs *avinodes.AviVsNode, svcName string) {
