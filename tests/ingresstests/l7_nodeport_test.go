...
func SetUpTestForIngressInNodePortMode(t *testing.T, model_Name string) {
	...
	testlib.CreateSVC(t, "default", "avisvc", corev1.ServiceTypeNodePort, false)
}
...
func TestMultiIngressToSameNodePortSvcInNodePort(t *testing.T) {
	...
	svcExample := (testlib.FakeService{
		...
		ServicePorts: []testlib.Serviceport{{PortName: "foo", Protocol: "TCP", PortNumber: 8080, TargetPort: 8080, NodePort: nodePort}},
	}).Service()
...
func TestMultipleNodeCreationAndDeletionInNodePort(t *testing.T) {
	...
	defer testlib.SetClusterIPMode()
	nodeIP1 := "10.1.1.2"
	testlib.CreateNode(t, "testNodeNP1", nodeIP1)
	...
	nodeIP2 := "10.1.1.20"
	testlib.CreateNode(t, "testNodeNP2", nodeIP2)
	defer testlib.DeleteNode(t, "testNodeNP2")
	testlib.PollForCompletion(t, modelName, 5)
	found, aviModel = objects.SharedAviGraphLister().Get(modelName)
	if found {
		nodes := aviModel.(*avinodes.AviObjectGraph).GetAviVS()
	...
	testlib.DeleteNode(t, "testNodeNP1")
	testlib.PollForCompletion(t, modelName, 5)
	found, aviModel = objects.SharedAviGraphLister().Get(modelName)
	if found {
		nodes := aviModel.(*avinodes.AviObjectGraph).GetAviVS()
...
func TestMultiPortServiceIngressInNodePort(t *testing.T) {
	...
	modelName := "admin/cluster--Shared-L7-0"
	objects.SharedAviGraphLister().Delete(modelName)
	testlib.CreateSVC(t, "default", "avisvc", corev1.ServiceTypeNodePort, true)
	ingrFake := (testlib.FakeIngress{
		Name:        "ingress-multipath",
		Namespace:   "default",
		DnsNames:    []string{"foo.com"},
...
func TestUpdateNodeInNodePort(t *testing.T) {
	...
	nodeIP := "10.1.1.2"
	nodeName := "testNodeNP"
	testlib.CreateNode(t, nodeName, nodeIP)
	defer testlib.DeleteNode(t, "testNodeNP")
...
func TestDeleteNodeInNodePort(t *testing.T) {
	...
	// Delete the Node
	testlib.DeleteNode(t, "testNodeNP")
