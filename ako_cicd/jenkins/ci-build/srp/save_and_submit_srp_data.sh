...
echo $IMAGE_DIGEST
docker manifest inspect $CI_REGISTRY_IMAGE_AKO/ako:${build_version} --insecure > ako_manifest.json
...
echo $IMAGE_DIGEST
docker manifest inspect $CI_REGISTRY_IMAGE_AKO/ako-operator:${build_version} --insecure > ako_operator_manifest.json
...
if [ "$minor_version" -ge "11" ]; then
    ...
    echo $IMAGE_DIGEST
    docker manifest inspect $CI_REGISTRY_IMAGE_AKO/ako-gateway-api:${build_version} --insecure > ako_gateway_api_manifest.json
