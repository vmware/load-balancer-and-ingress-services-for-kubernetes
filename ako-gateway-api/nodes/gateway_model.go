...
func BuildVsVipNodeForGateway(gateway *gatewayv1.Gateway, parentVsNode *nodes.AviEvhVsNode, infraSetting *v1beta1.AviInfraSetting, key string) *nodes.AviVSVIPNode {
	...
	gwClassName := string(gateway.Spec.GatewayClassName)
	vsvipNode.EnablePublicIP = proto.Bool(akogatewayapiobjects.GatewayApiLister().IsGatewayClassVipTypePublic(gwClassName))
	return vsvipNode
}
