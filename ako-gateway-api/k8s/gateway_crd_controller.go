...
func (c *GatewayController) processHTTPRoutes(key, namespace, name string, numWorkers uint32) {
	...
	case akogatewayapilib.RouteBackendExtensionKind:
		...
		for httpRoute := range httpRoutes {
			...
			ok := routeBackendExtensionNSNameList[routeBackendExtensionNSName]
			if !ok {
				// IF HTTPRoute to RouteBackendExtension Mapping is no there.. delete the entry from RouteBackendExtension to HTTPRoute
