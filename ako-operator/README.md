...
<i>**Step 4**</i>: Click `install` and select the 1.9.1 version. The operator will be installed in `avi-system` namespace. The namespace will be created if it doesn't exist.
...
```
helm template ako/ako-operator --version 1.9.1 --include-crds --output-dir <output_dir>
```
...
```
helm show values ako/ako-operator --version 1.9.1 > values.yaml
```
...
```
helm upgrade <release-name> ako/ako-operator -f /path/to/values.yaml --version 1.9.1 --namespace=avi-system
```
