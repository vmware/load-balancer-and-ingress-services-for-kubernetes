...
func createOrUpdateClusterRole(ctx context.Context, ako akov1beta1.AKOConfig, log logr.Logger, r *AKOConfigReconciler) error {
	...
	if oldCR.GetName() != "" {
		log.Info("old cr", "rules", utils.Stringify(oldCR.Rules))
		log.Info("new cr", "rules", utils.Stringify(cr.Rules))
		...
		oldCR.Rules = cr.Rules
		err := r.Update(ctx, &oldCR)
		if err != nil {
			log.Error(err, "unable to update clusterrole", "name", oldCR.GetName())
			return err
		}
	} else {
		err := r.Create(ctx, &cr)
		if err != nil {
			log.Error(err, "unable to create clusterrole", "name", cr.GetName())
			return err
		}
	}
	...
	if err != nil {
		log.Error(err, "unable to get clusterrole", "name", getCRName().Name)
		return err
	}
	log.V(0).Info("new CR get", "rules", utils.Stringify(newCR.Rules))
...
func BuildClusterrole(ako akov1beta1.AKOConfig, r *AKOConfigReconciler, log logr.Logger) rbacv1.ClusterRole {
