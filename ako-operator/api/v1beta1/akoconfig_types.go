/*
Copyright 2019-2025 Broadcom, Inc.
All Rights Reserved.
...
type NetworkSettings struct {
	...
	// 1. static routes are disabled
	// 2. non vcenter clouds
	...
	//For L7 VSes(created from OpenShift Routes), if spec.subdomain field is specified instead of spec.host field for an OpenShift route, then the default domain specified is appended to the spec.subdomain to form the FQDN for the VS. The defaultDomain should be configured as a sub-domain in Avi cloud.
...
type L7Settings struct {
	...
	// FQDNReusePolicy can be used control whether AKO allows cross-namespace usage of FQDNs
	FQDNReusePolicy FQDNReusePolicy `json:"fqdnReusePolicy,omitempty"`
}

// L4Settings defines the L4 configuration for the AKO controller
type L4Settings struct {
	// If multiple sub-domains are configured in the cloud, use this knob to set the default sub-domain to use for L4 VSes. This flag will be deprecated in a future release; use NetworkSettings.defaultDomain instead.
	// If both NetworkSettings.defaultDomain and L4Settings.defaultDomain are set, then NetworkSettings.defaultDomain will be used.
	DefaultDomain string `json:"defaultDomain,omitempty"`
...
type FeatureGates struct {
	...
	// EnableEndpointSlice enables/disables processing of EndpointSlices instead of Endpoints in AKO. This field defaults to true if not set.
...
// +operator-sdk:csv:customresourcedefinitions:displayName="AKOConfig"
// +operator-sdk:csv:customresourcedefinitions:resources={{Statefulset,v1,""},{Configmap,v1,""}}
